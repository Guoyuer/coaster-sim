...
bool FYarlungCorridorProfileEnvelopeTest::RunTest(const FString& Parameters)
{
    ...
    TestEqual(
        ...
        0.0f);
    TestEqual(
        TEXT("Far corridor terrain profile does not use the ride envelope"),
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
    FYarlungCorridorProfileTerrainHeightTest,
    "CoasterSim.Yarlung.CorridorProfile.TerrainHeight",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FYarlungCorridorProfileTerrainHeightTest::RunTest(const FString& Parameters)
{
    const FVector2D Center(12000.0f, -34000.0f);
    const float TrackBaseHeight = 360000.0f;
    const float BaseHeight = 346000.0f;

    const float NearHeight = YarlungCorridorProfile::CorridorTerrainHeightCm(Center, 24000.0f, TrackBaseHeight, BaseHeight);
    const float WallHeight = YarlungCorridorProfile::CorridorTerrainHeightCm(Center, 150000.0f, TrackBaseHeight, BaseHeight);
    const float RaisedBaseWallHeight = YarlungCorridorProfile::CorridorTerrainHeightCm(Center, 150000.0f, TrackBaseHeight, BaseHeight + 50000.0f);

    TestTrue(TEXT("Corridor terrain height is finite"), FMath::IsFinite(NearHeight) && FMath::IsFinite(WallHeight));
    ...
    TestTrue(
        TEXT("Corridor profile preserves broad DEM elevation deltas"),
