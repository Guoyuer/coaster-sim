...
- **⚠️ 定位已变更（2026-06-19 用户拍板；2026-06-21 拓扑放宽）**：项目目标从「峡谷里的小设施」重定位为 **世界最长(~5km)风景优先实景过山车 + 真物理 + 照片级第一人称**。轨道不再限定沿江往返，可跨江、高架俯视、贴崖 overbank、俯冲或短暂翻滚；详见 **`docs/plans/worlds-longest-coaster.md`**（codex-ready 设计 spec）。本「画质重构计划」的走廊/轨道范围受其约束；冲突时以 worlds-longest spec 的定位与优先级为准。
...
- **S-D. 照片自然性 > DEM 忠实**：公开 30m DEM 是大轮廓参考，不是必须逐米保真的成品地形。若 DEM/r16-derived corridor terrain 在第一人称近景读成货架、硬墙、人工切槽，必须作者化重塑为自然坡面或用 Nanite cliff/forest assets 接管。**不要挖山适配轨道；让轨道高度/路径适配自然地形。**
...
```
该脚本顺序：① `generate-yarlung-corridor-source-assets.py` 生成 DEM/r16 源数据、preview、masks、river CSV → ② `generate-yarlung-track.py` 生成并校验 `YarlungTrack.csv` → ③ `create-coaster-materials.py` 建当前运行时材质 → ④ `-run=YarlungCorridorImport` commandlet 构建 `SM_YarlungCorridorTerrain`、显式 `SM_YarlungRiverSurface`、scenery 并重建 `.umap`。
- **关卡 (.umap) 是 commandlet 生成物，不是手摆的。** 要往关卡加/删 actor，改 `YarlungCorridorImportCommandlet.cpp` 再重跑，否则手改会被下次导入覆盖。
- headless UE 脚本即使内部报错也可能返回 exit code 0 → 必须用 success-marker / 资产存在性校验兜底（已有先例，保持）。

**画面验证（截图回归——这就是验收标准，因为它抓的正是真实第一人称画面）：**
```powershell
.\scripts\offscreen-shot.ps1 -Build -Name "hero-stepN" -ResX 2560 -ResY 1440 -JumpSeconds 12
```
...
generate-yarlung-corridor-source-assets.py ──> YarlungTsangpo_1009.r16 + preview/masks + YarlungRiver.csv
generate-yarlung-track.py             ──> YarlungTrack.csv + manifest track block
create-coaster-materials.py           ──> M_YarlungMeshTerrain / river / coaster / imported live PBR material usage flags
YarlungCorridorImportCommandlet.cpp   ──> 读 .r16/CSV → build SM_YarlungCorridorTerrain + spawn scenery actors → 存 .umap
...
- **S1（当前已清理大半，仍是天花板约束）low-ceiling 几何不能回流**：旧顶点色假天穹/假云/假远山、旧 procedural river ribbon、旧 canyon-wall 幕布、隐藏 `TrainBody` cube placeholder 已从默认链路删除。当前仍保留的 engine BasicShapes 只用于圆柱 rails/ties/supports 快速可见结构；最终 D2 仍要替换成 authored 钢轨/支撑/车体资产。
...
- **S3 轨道/支撑仍是运行时圆柱结构，车体缺失**：rails/ties/supports 的可见几何已拆到 `CoasterTrackVisuals`，但仍是运行时 cylinder 管轨；隐藏车体 placeholder 已删除，当前没有 authored cockpit/train/安全杆。相对山体/江水不是第一优先级，但 D 阶段不能标 Done。
- **S4 江水仍低保真**：当前水体是 commandlet 生成的显式 `SM_YarlungRiverSurface`，读取 `YarlungRiver.csv` 和 `Config/yarlung-assets.json` 的 water 配置生成；材质/flow/depth/reflection 仍不足，但不要恢复旧 procedural river actor 或 gate-only UE Water actor。
...
8. 资产授权：仅 CC0 / license-compatible（Fab-Megascans 随 UE 免费 / 真实公开 DEM / 项目明确允许的本地免费资产）。禁止商业模拟器资产。
...
- **0.1 选英雄段**：在轨道样条上选一段**最如画**的弧——不再只限临江高弯，也可以是跨江、高架俯视、贴崖 overbank、俯冲出谷或短暂 roll 后的开阔段；硬目标是同一帧能看到：脚下/侧下方江水、对岸崖壁、森林坡、远处雪山+云带。用 `SampleFrame` 在若干 `TrackRatio` 上打印相机位置/朝向，挑出取景最好的 2–3 个 `JumpSeconds` 时间点固定下来（之后 offscreen-shot 只看这几个时间点）。
...
- **A1 删假环境几何（已完成，禁止回归）**：`BuildSkyDome`/`BuildCloudLayer`/`BuildDistantRidges`、旧 `BuildRiver*` ribbon、旧 canyon-wall 幕布、隐藏 `TrainBody` cube placeholder 均已从默认链路删除。保留仿真、相机、运行时 rails/ties/supports 和 generated level actors。
  - 验收：编译通过；FP 截图不再有方块云/顶点色天穹/平面远山/旧 ribbon 水体；天空交给 `SkyAtmosphere` + `VolumetricCloud`，江水交给显式 `SM_YarlungRiverSurface`。
- **A2 真实 DEM 地形（如画轮廓的来源）— 真实尺度，不压缩**（2026-06-18 决策）：
  ...
  - **过山车作为峡谷里的风景叙事线**：占地约 150m 的早期短环已被世界最长约 5km scenic layout 取代。轨道样条坐标由生成器写入 `YarlungTrack.csv`，路线可以沿江、跨江、高架俯视、贴崖 overbank 或俯冲/翻滚；核心是让第一人称样条穿过取景最好的峡谷构图，而不是被沿江往返几何绑死。
  - **选定子区域（2026-06-18 用户拍板）=大拐弯最深峡谷段**，南迦巴瓦与加拉白垒之间（世界最深，最大深度 6009m）。险峻+如画。
    ...
    - **垂直 1:1 真实尺度（关键）**：把 `YarlungCorridorImportCommandlet.cpp` 的 `EncodedMinZ/EncodedMaxZ` 从 `-360cm/3900cm` 改成框内**真实海拔范围**（当前编码窗口 **2600m–7300m**，注意单位 cm），让崖壁是几百上千米而非 43m。`XYScale` 由 bbox 自动算出（当前 X≈6.70m/quad、Y≈8.27m/quad，可接受）。不要再依赖 clearance-cut 挖山；轨道净空由生成器 + `verify-track-clearance.py` 门禁保证。
    - **保险步骤**：正式导入前先出一张该 bbox 的 hillshade（晕渲）预览图肉眼确认河道走向与雪峰位置，避免下错瓦片/朝向不对白跑一轮。
  - 远景"画意"主要来自真实山体轮廓+真实尺度——这一步价值最高。
  - **照片向自然化（2026-06-19 转向）**：若 DEM 近景陡崖或轨道净空切槽在 FP 帧中读成水平货架/人工墙，允许并应当 naturalize DEM/r16 source grid、改轨道 Z/XY、重塑走廊地形，或用 Nanite cliff/forest assets 接管近中景。DEM 保留中远景大轮廓；近景以照片自然性为准。
...
- **A3 scatter 改查真实地形，并迁出 ride actor**：用 corridor mesh / `.r16` / track CSV 的同一数据契约放置巨石/植被，替换对解析 `YarlungLandscapeHeight()` 的依赖。首选在 `YarlungCorridorImportCommandlet.cpp` 或专门的 generated scenery/PCG 管线中生成关卡内容；若必须用运行时 `LineTraceSingleByChannel`（高空向下打 `ECC_WorldStatic`），也要封装到独立 scenery actor，不要继续写进 `ACoasterRideActor::RebuildEnvironment`。注意 commandlet 离屏环境与 PIE/运行时都要验证。最终删除影子地形函数。
...
- **C1 corridor terrain material / asset layer**：继续改 live 的 `M_YarlungMeshTerrain` 生成逻辑与 scenery actor，而不是恢复 `create_landscape_material` / `M_YarlungLandscapeGround` / macro TGA。远中景需要大尺度 canyon wall / mountainside 形体，近中景叠 Megascans/扫描 PBR 做 normal/roughness/AO/breakup，按坡度/高度/湿度分层（河岸/森林/岩石/雪/湿岩带）。法线和位移必须宏观/微观尺度分离，避免重现 moire（bug 日志教训）。必要时用独立 Nanite cliff/forest asset 接管掠射崖面。
  - 验收：崖壁/坡面有真实岩石/植被形体和法线起伏，颜色冷灰绿（非橙）；远看无可见平铺；第一人称全程不出现 DEM-derived stair-step / 大块幕墙。

### 阶段 D — 真实资产替换（江水 > 轨道/车）
- **D1 真实江水（远中景主角，优先）**：基于显式 `SM_YarlungRiverSurface` 的水体材质——深度、折射/透明感、流动法线、Lumen/SSR 反射、湍流泡沫，乳白—青绿色（`CONTEXT.md`）。替换当前仅靠顶点色的水面读感。
  - 验收：江面有反射/折射/流动/岸边泡沫，非卡通青平面。
- **D2 轨道/支撑/车 → 真实网格 + 金属 PBR**：当前 rails/ties/supports 是运行时 cylinder 管轨，已经不再有隐藏 cube 车体。下一步应做扫掠钢轨剖面 + 枕木/螺栓次级细节（Nanite）、支撑钢结构件、authored cockpit/train/安全杆；从 `M_CoasterTint` 平涂换金属 PBR（normal/roughness/metallic/磨损）。
...
### 阶段 F — 沿轨道铺开
- 英雄段达标后，沿整条样条逐段验证（多个 `JumpSeconds`），补齐走廊内其余区段；走廊外保持低模/裁剪。
...
- **TSR vs 截图**：offscreen-shot 使用 UE 渲染输出路径，仍需确认能反映 TSR 后的画面（必要时切换脚本的 `-CaptureMode ImmediateHighResShot` 或经 `-ExecCmds` 出图）。
...
- 关卡导入 commandlet：`Source/CoasterSim/YarlungCorridorImportCommandlet.cpp`
- GameMode：`Source/CoasterSim/CoasterGameMode.cpp`
- 材质/贴图生成：`scripts/create-coaster-materials.py`
- 高度图/river/mask 生成：`scripts/generate-yarlung-corridor-source-assets.py`
- 管线编排：`scripts/import-yarlung-corridor.ps1`
