...
### 第五轮清理（2026-06-20）— 删除运行时已死的 landscape 材质链

corridor-mesh pivot 后 Landscape actor 已退役（`landscape_count=0`），`M_YarlungLandscapeGround` 及其专属 grass/macro 贴图链全程无运行时引用。整条死链删除：

- **`create-coaster-materials.py`**：删 `create_landscape_material`（~180 行）+ `LEAFY_GRASS_*`/`YARLUNG_MACRO_*` 常量/纹理集 + `main()` 里对应的 ensure_folder/import/调用。
- **`generate-yarlung-landscape-assets.py`**：删 macro TGA 生成——`macro_albedo`/`macro_roughness`/`write_tga` 函数、三个 macro 像素列表、循环内三处 append、三处 `write_tga` 调用、manifest 三个 macro 字段、`--texture-out` 参数 + `texture_out_dir`、README 三行。heightmap(.r16) 计算路径未动，输出可证不变。
- **`import-yarlung-landscape.ps1`**：构建 marker 从死的 `M_YarlungLandscapeGround` 重指到 live `M_YarlungMeshTerrain`；删 `$MacroTextureSources`/`$MacroTextureAssets` 新鲜度数组。
- **`inspect-yarlung-map.py`**：删 landscape 材质的 load/emit 诊断段。
- **删除文件**：`inspect-yarlung-material.py`、`export-yarlung-macro-texture.py`（纯诊断死链）；8 个死 `.uasset`（`M_YarlungLandscapeGround` + LeafyGrass×4 + YarlungMacro×3）。

净删 ~294 行脚本 + 8 个 LFS 资产 + 2 个脚本文件。验证：四脚本 `py_compile`/argparse PASS；`-ForceMaterials` 重生成 PASS（marker 重指生效）；offscreen `deadcode-cleanup-verify` 已 Read，live 场景（地形/轨道/水/云）与删除前完全一致——证明该链确实运行时已死。

剩余（更激进、未做）：对应的 SourceAssets 输入（`SourceAssets/PolyHaven/leafy_grass/*.jpg`、`SourceAssets/Generated/YarlungLandscape/*macro*.tga`）现在也无人消费，可作为 source 侧清理单独评估。
