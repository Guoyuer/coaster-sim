...
## 当前状态
- **当前阶段**：阶段 A/B 交叉 — A1 Done；A2 **参数层已验收通过**；**B2 颜色根因（橙黄）已验收通过**，蓝天/绿地恢复。但 B2 **曝光标定未完**（偏暗，像黄昏不像晴天），且独立验收发现 **A2 地形有"梯田台阶"疑似 8-bit 高程量化**——这是当前挡住 D1 的真问题。
...
- **下一步动作（按顺序，两步）**：
  1. **收尾 B2 曝光**（便宜，先做）：当前物理曝光 f/11·1/500·ISO100 ≈ **EV≈15.9**，比晴天 sunny-16(~EV14.6) 暗约 1.2 档。把场景调亮到晴天：`AutoExposureBias` 0.0→**+1.0~+1.3**（或 ISO100→250 / 快门500→200 任一）。目标 EV~14.5–15，蓝天通透、岩壁见光、明暗对比出来。重截 hero 验 D2≥4。
  2. **修 A2 高程台阶**（真正卡 D1）：核查 `YarlungTsangpo_1009.r16` 与导入链是不是被压成 8-bit/posterize（256 级摊 4700m ≈ 18m/台阶，和画面台阶高度吻合）。确认是真 16-bit 后台阶应消失，才可验 D1≥4 标 A2 Done。
...
| B | 物理天空+光照标定 | 🟦 进行中 | 单一物理天空+大气透视/曝光物理化/D2≥4 D5≥3 | **B2 橙黄根因已验收 PASS**；曝光偏暗(EV≈15.9)未收尾；仍缺云带/大气透视/材质层次 |
...
- [ ] **B2 曝光收尾（下一步①，便宜先做）**：当前 EV≈15.9 偏暗。调亮到晴天 EV~14.5–15（`AutoExposureBias`+1.0~1.3 或 ISO100→250 或快门500→200）。重截 hero `WaitSeconds 12` 验 D2≥4。
