...
- **S-D. 照片自然性 > DEM 忠实**：公开 30m DEM 是大轮廓参考，不是必须逐米保真的成品地形。若真实 DEM/单值 heightfield 在第一人称近景读成货架、硬墙、人工切槽，必须作者化重塑为自然坡面。**不要挖山适配轨道；让轨道高度/路径适配自然地形。**
...
```
该脚本顺序：① `generate-yarlung-landscape-assets.py` 生成高度图 `.r16` / preview / masks / river CSV → ② `create-coaster-materials.py` 建当前运行时材质 → ③ `-run=YarlungLandscapeImport` commandlet 重建 `.umap` 与 corridor scenery。
