...
```
该脚本顺序：① `generate-yarlung-corridor-source-assets.py` 生成 DEM/r16 源数据、preview、masks、river CSV → ② `generate-yarlung-track.py` 生成并校验 `YarlungTrack.csv` → ③ `create-coaster-materials.py` 建当前运行时材质 → ④ `-run=YarlungCorridorImport` commandlet 构建 `SM_YarlungCorridorTerrain`、UE Water、scenery 并重建 `.umap`。
...
- **S4 江水仍低保真**：当前水体是 commandlet 生成的 UE Water `WaterBodyRiver`，由 `YarlungWaterBuilder` 读取 `YarlungRiver.csv` 和 `Config/yarlung-assets.json` 的 water 配置生成；材质/flow/depth/reflection 仍不足，但不要恢复旧 procedural river actor。
...
- **A1 删假环境几何（已完成，禁止回归）**：`BuildSkyDome`/`BuildCloudLayer`/`BuildDistantRidges`、旧 `BuildRiver*` ribbon、旧 canyon-wall 幕布、隐藏 `TrainBody` cube placeholder 均已从默认链路删除。保留仿真、相机、运行时 rails/ties/supports 和 generated level actors。
  - 验收：编译通过；FP 截图不再有方块云/顶点色天穹/平面远山/旧 ribbon 水体；天空交给 `SkyAtmosphere` + `VolumetricCloud`，江水交给 UE Water。
