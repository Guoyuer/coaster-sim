...
```
该脚本顺序：① `generate-yarlung-landscape-assets.py` 生成 DEM/r16 源数据、preview、masks、river CSV → ② `generate-yarlung-track.py` 生成并校验 `YarlungTrack.csv` → ③ `create-coaster-materials.py` 建当前运行时材质 → ④ `-run=YarlungLandscapeImport` commandlet 构建 `SM_YarlungCorridorTerrain`、UE Water、scenery 并重建 `.umap`。
