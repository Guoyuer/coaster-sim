...
3. 估算几何 G：由相邻采样的曲率 + 设计速度估 `vert_g/lat_g`，坡度→`long_g`。
...
- 横向 G：`|lat_g| ≤ 1.7`（允许更利落的横向冲击；生成器可适度欠 bank 制造侧向刺激）
- 纵向 G：`|long_g| ≤ 2.5`
（「刺激但仍真人可乘」：真实过山车持续上限 ~5–6G、ejector airtime ~-1~-1.5G，本档落在其内。若实测过猛/过软，在 P1 校验器参数里收放。）

**P1 draft 例外（2026-06-19 用户拍板「急一点也 ok」）**：P1 首版离线线路允许用更宽的横向 G 估算门槛（当前 `verify-track-clearance.py --max-lat-g` 默认 `3.8`）先放行 scenic 5km centerline + 净空证据；P3 仍要通过曲率 banking、速度调参、turnaround 重塑把运行时遥测收回目标舒适包络。也就是说：P1 的 `3.8` 是生成器草案门槛，不是最终乘坐舒适目标。
...
1. ✅ **舒适包络 = 更刺激**：见 §5 更新值（垂直 `[-1.5,+5.5]`、横向 `≤1.7`、纵向 `≤2.5`，airtime 丘 ejector `[-1.2,+0.2]`）。
